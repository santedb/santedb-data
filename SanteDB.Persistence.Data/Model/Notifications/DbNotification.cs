/*
 * Copyright (C) 2021 - 2025, SanteSuite Inc. and the SanteSuite Contributors (See NOTICE.md for full copyright notices)
 * Copyright (C) 2019 - 2021, Fyfe Software Inc. and the SanteSuite Contributors
 * Portions Copyright (C) 2015-2018 Mohawk College of Applied Arts and Technology
 * 
 * Licensed under the Apache License, Version 2.0 (the "License"); you 
 * may not use this file except in compliance with the License. You may 
 * obtain a copy of the License at 
 * 
 * http://www.apache.org/licenses/LICENSE-2.0 
 * 
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
 * WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the 
 * License for the specific language governing permissions and limitations under 
 * the License.
 * 
 */
using System;
using SanteDB.OrmLite.Attributes;
using SanteDB.Persistence.Data.Model.Concepts;

namespace SanteDB.Persistence.Data.Model.Notifications
{
    /// <summary>
    /// Represents a notification.
    /// </summary>
    [Table("notification_tbl")]
    public class DbNotification : DbNonVersionedBaseData
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="DbNotification"/> class.
        /// </summary>
        public DbNotification()
        {

        }

        /// <summary>
        /// Gets or sets the key of the notification.
        /// </summary>
        [Column("id"), PrimaryKey, AutoGenerated]
        public override Guid Key { get; set; }

        /// <summary>
        /// Gets or sets the template id of the notification.
        /// </summary>
        [Column("tpl_id"), ForeignKey(typeof(DbNotificationTemplate), nameof(DbNotificationTemplate.Key))]
        public Guid TemplateId { get; set; }

        /// <summary>
        /// Gets or sets the entity type of the notification.
        /// </summary>
        [Column("ent_typ_cd_id"), ForeignKey(typeof(DbConcept), nameof(DbConcept.Key))]
        public Guid EntityType { get; set; }

        /// <summary>
        /// Gets or sets the mnemonic of the notification.
        /// </summary>
        [Column("mnemonic")]
        public String Mnemonic { get; set; }

        /// <summary>
        /// Gets or sets the name of the notification.
        /// </summary>
        [Column("name")]
        public String Name { get; set; }

        /// <summary>
        /// Gets or sets the state of the notification.
        /// </summary>
        [Column("state_cd_id"), ForeignKey(typeof(DbConcept), nameof(DbConcept.Key))]
        public Guid State { get; set; }

        /// <summary>
        /// Gets or sets the description of the notification.
        /// </summary>
        [Column("descr")]
        public String Description { get; set; }

        /// <summary>
        /// Gets or sets the filter expression of the notification.
        /// </summary>
        [Column("fltr_expr")]
        public String FilterExpression { get; set; }

        /// <summary>
        /// Gets or sets the target expression of the notification.
        /// </summary>
        [Column("trg_expr")]
        public String TargetExpression { get; set; }

        /// <summary>
        /// Gets or sets the trigger expression of the notification.
        /// </summary>
        [Column("trgr_expr")]
        public String TriggerExpression { get; set; }

        /// <summary>
        /// Gets or sets the last sent at date time of the notification.
        /// </summary>
        [Column("last_sent_utc")]
        public DateTime LastSentAt { get; set; }
    }
}
