...
ALTER TABLE ACT_VRSN_UPD_TBL add CLS_CD_ID UUID;
ALTER TABLE ACT_VRSN_UPD_TBL add TPL_ID UUID;
ALTER TABLE ACT_VRSN_UPD_TBL add MOD_CD_ID UUID;
...
ALTER TABLE act_vrsn_tbl ADD CONSTRAINT ck_act_vrsn_mod_cd CHECK (is_cd_set_mem(mod_cd_id, 'ActMood'::character varying));
ALTER TABLE ent_vrsn_tbl ADD CONSTRAINT ck_ent_vrsn_dtr_cd CHECK (is_cd_set_mem(dtr_cd_id, 'EntityDeterminer'::character varying));
...
CREATE TABLE GEO_TBL (
	GEO_ID UUID NOT NULL DEFAULT uuid_generate_v1(),
	LAT REAL NOT NULL,
	LNG REAL NOT NULL,
